...
const SkillParentSelection = ({ name, onChange, skill_id }) => {
      ...
      if (skill_id) {
        try {
          const response = await axios.get(
            `http://127.0.0.1:5000/api/v1/skills/${skill_id}/parents`
          );
      ...
      try {
        const response = skill_id
          ? // Get the possible parents for the skill if we are editing
            await axios.get(
              `http://127.0.0.1:5000/api/v1/skills/${skill_id}/possible_parents`
            )
  ...
  const handleAddParent = (parent) => {
    setCurrentParents((prevCurrentParents) =>
      [...prevCurrentParents, parent].sort((a, b) =>
        a.name.localeCompare(b.name)
      )
    );
    setPossibleParents((prevPossibleParents) =>
      prevPossibleParents
        .filter((item) => item.name !== parent.name)
  ...
  const handleRemoveParent = (parent) => {
    setPossibleParents((prevPossibleParents) =>
      [...prevPossibleParents, parent].sort((a, b) =>
        a.name.localeCompare(b.name)
      )
    );
    setCurrentParents((prevCurrentParents) =>
  ...
  return (
    <div className="flex flex-col">
      <InputLabel label="Add a Skill Parent" name="currentParents" />
      <FilterList
      ...
      <InputLabel label="Remove a Skill Parent" name="possibleParents" />
      <FilterList
        list={currentParents}

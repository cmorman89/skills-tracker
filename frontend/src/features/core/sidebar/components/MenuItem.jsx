...
const MenuItem = ({ collapsed, icon, label, level, onClick, url }) => {
    ...
    return (
        <div
            ...
            onClick={handleClick}
        >
            <div className={`transition-all text-xl ease-in-out duration-400 ${getIndent()}`}>
                <FontAwesomeIcon icon={icon} />
            </div>
            <span className={`transition-all block text-lg whitespace-nowrap ${collapsed ? 'opacity-0 ' : 'opacity-100 '} ease-in-out duration-400`}>
                {label}
            </span>
        </div>
    );
